/**
 *  MediaRenderer Service Manager v 2.1.0  Audio Server
 *
...
def addMediaRenderer() {
	...
	selectedMediaRenderer.each { dni ->
		...
		if(!d) {
			...
			if (newPlayer){
				d = addChildDevice("jybrock", "DLNA Player", dni, newPlayer?.value.hub, [label:"${newPlayer?.value.name} Speaker","data":["model":newPlayer?.value.model,"avtcurl":newPlayer?.value.avtcurl,"avteurl":newPlayer?.value.avteurl,"rccurl":newPlayer?.value.rccurl,"rceurl":newPlayer?.value.rceurl,"udn":newPlayer?.value.udn,"dni":dni]])
			}
			runSubscribe = true
		} 
	}
